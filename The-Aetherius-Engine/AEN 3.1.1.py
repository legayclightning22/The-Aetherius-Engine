...
class Visualizer(tk.Tk):
    def __init__(self, kernel: AetheriusKernel):
        ...
        # Singularity Gauge
        tk.Label(self.sidebar, text="SINGULARITY CHARGE", fg="#aa00ff", bg="#0f0f12", font=("Consolas", 9)).pack(anchor="w")
    ...
    def animate(self):
        ...
        if self.kernel.singularity["enabled"]:
            # Visual precursor
            if charge > 0.5:
                # Faint distortion/pull lines
                for _ in range(int(charge * 5)):
                    angle = random.uniform(0, 6.28)
                    dist = random.uniform(50, 300)
                    fx = radar_x + math.cos(angle) * dist
                    fy = radar_y + math.sin(angle) * dist
                    self.canvas.create_line(fx, fy, radar_x, radar_y, fill="#330044", width=1, dash=(2,4))

            if self.kernel.singularity["active"] or charge > 0.8:
