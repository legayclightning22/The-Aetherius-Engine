    ...
    async def run_simulation_loop(self):
        ...
        while self.running:
            ...
            if not self.singularity["active"]:
                # Slowly charge singularity based on entropy/instability
                growth = 0.003 if self.manual_instability else 0.0008
                self.singularity["charge"] = min(self.singularity["charge"] + growth, 1.0)
                
                if self.singularity["charge"] >= 1.0:
                    self.singularity["active"] = True
                    self.singularity["phase"] = "ACCRETION"
                    self.singularity["timer"] = 180 # Frames
...
class Visualizer(tk.Tk):
    def __init__(self, kernel: AetheriusKernel):
        ...
        self._create_metric_row("SINGULARITY", self.metrics_vars["charge"], "#aa00ff")
    ...
    def animate(self):
        ...
        # --- SINGULARITY VISUALS ---
        if self.kernel.singularity["active"] or self.kernel.singularity["charge"] > 0.8:
            charge = self.kernel.singularity["charge"]
            radius = 50 * charge + (random.randint(-2, 2) if charge > 0.9 else 0)
        ...
        self.metrics_vars["charge"].set(f"{int(self.kernel.singularity['charge'] * 100)}%")
