...
class AetheriusKernel:
    ...
    def trigger_instability(self):
        """Injects massive entropy into the system."""
        self.log("WARNING: MANUAL INSTABILITY TRIGGERED!")
        self.metrics["status"] = "SYSTEM UNSTABLE"
        for node in self.nodes.values():
            # Chaotic velocity boost
            node.dx += random.uniform(-8.0, 8.0)
            node.dy += random.uniform(-8.0, 8.0)
            # Spike energy
            node.energy = 1.0
            # Force random state changes
    ...
    def _update_metrics(self, cycle_id: int):
        ...
        # Only overwrite status if not currently unstable from user input
        if self.metrics["status"] != "SYSTEM UNSTABLE":
...
class Visualizer(tk.Tk):
    def __init__(self, kernel: AetheriusKernel):
        ...
        tk.Label(self.sidebar, text="[SPACE] Destabilize", fg="#555", bg="#0f0f12", font=("Consolas", 8)).pack(anchor="w", pady=(0,10))
    ...
    def on_space(self, event):
        self.kernel.trigger_instability()
